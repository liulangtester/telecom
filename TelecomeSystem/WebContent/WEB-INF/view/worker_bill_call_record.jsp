...
<head>
	...
	var get = function(){
// 			alert($("#get").val())
			window.location = "${pageContext.request.contextPath}/workerBillServlet?method=callRecord&search="+$("#get").val();
	};
</script>
</head>
		...
		<div id="page-wrapper" class="gray-bg" >
			...
			<div class="row clearfix">
				<div class="col-md-12 column">
                    ...
                    <div class="col-sm-12">
                        <h4 class="example-title">通话记录查询</h4>
                        ...
                        <div class="example">
                            <table id="bill-table" data-mobile-responsive="true">
                                    ...
                                    <tr>
                                    	<td><input onclick="check()" id="checked" class="checkbox"  type="checkbox"/> </td>
                                    	<th data-field="phone_id">账单号</th>
                                        <th data-field="phone_num">呼叫号码</th>
                                        <th data-field="bill_beCalled_phone_num">被叫号码</th>
                                        <th data-field="bill_time">通话时长</th>
                                        <th data-field="bill_date">通话日期</th>
                                        <th data-field="bill_money">金额</th>
                                        
                                     </tr>   
                                     
<!--                                         迭代数据 -->
											<c:forEach items="${bill}" var="b" varStatus="status">
                                        <tr class="field" style="border-bottom:#f3d5d6 1px dashed;">
                                        	<td><input id="checkbox" class="checkbox" value="${b.id}" id="box_${b.id}" type="checkbox"/> </td>                                 
							            	<td>${b.id}</td>
							            	<td>${b.callNumber}</td>
							                <td>${b.beCalledNumber}</td>
							                <td>${b.time}</td>
							                <td>${b.date}</td>
							                <td>${b.cost}</td>
...
<!-- 	    	在第一页，且不止一页，首页，上一页不可点 -->
	    		【首页】
	    		【上一页】
	    		【<a href="${pageContext.request.contextPath}/workerBillServlet?method=callRecord&thisPage=${notices.pageNum+1}&search=${notices.search}">下一页</a>】
	    		【<a href="${pageContext.request.contextPath}/workerBillServlet?method=callRecord&thisPage=${notices.totalPage}&search=${notices.search}">尾页</a>】
	    	</c:if>
	    	    		
	    	<c:if test="${notices.pageNum>1 && notices.pageNum<notices.totalPage}">
<!-- 	    	全都可点 -->
	    		【<a href="${pageContext.request.contextPath}/workerBillServlet?method=callRecord&thisPage=1&search=${notices.search}">首页</a>】
	    		【<a href="${pageContext.request.contextPath}/workerBillServlet?method=callRecord&thisPage=${notices.pageNum-1}&search=${notices.search}">上一页</a>】
	    		【<a href="${pageContext.request.contextPath}/workerBillServlet?method=callRecord&thisPage=${notices.pageNum+1}&search=${notices.search}">下一页</a>】
	    		【<a href="${pageContext.request.contextPath}/workerBillServlet?method=callRecord&thisPage=${notices.totalPage}&search=${notices.search}">尾页</a>】
	    	</c:if>
...
<!-- 	    	在最后一页，下一页和尾页不可点 -->
    ...
    <script src="${pageContext.request.contextPath}/js/user_bill.js"></script>	
