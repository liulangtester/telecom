...
<head>
	...
	var get = function(){
// 			alert($("#get").val())
			window.location = "${pageContext.request.contextPath}/workerCustomerServlet?method=updateOrDeleteBig&search="+$("#get").val()+"&kind=updateOrDelete";
	};
	
	/** 删除信息 */
	var deleteFn = function(id){
		if (confirm("您确定要删除吗？")){
			window.location = "${pageContext.request.contextPath}/workerCustomerServlet?method=deleteBig&id=" + id;
		
	};
	}
	/** 修改信息 */
	var updateFn = function(id){
			window.location = "${pageContext.request.contextPath}/workerCustomerServlet?method=updateBig&id=" + id;	
	};
                            ...
                            <table id="bill-table" data-mobile-responsive="true">
                                    ...
                                    <tr>
                                        ...
                                        <th data-field="time">客户行业</th>
                                         <th data-field="time">电信专员</th>
                                        <th data-field="time">操作</th>
                                        <th data-field="time">操作</th>
                                    </tr>
                                    
                                    <!--                                         迭代数据 -->
											<c:forEach items="${bigCustomer}" var="r">
...
<!-- 	    	在第一页，且不止一页，首页，上一页不可点 -->
	    		【首页】
	    		【上一页】
	    		【<a href="${pageContext.request.contextPath}/workerCustomerServlet?method=updateOrDeleteBig&thisPage=${notices.pageNum+1}&search=${notices.search}">下一页</a>】
	    		【<a href="${pageContext.request.contextPath}/workerCustomerServlet?method=updateOrDeleteBig&thisPage=${notices.totalPage}&search=${notices.search}">尾页</a>】
	    	</c:if>
	    	    		
	    	<c:if test="${notices.pageNum>1 && notices.pageNum<notices.totalPage}">
<!-- 	    	全都可点 -->
	    		【<a href="${pageContext.request.contextPath}/workerCustomerServlet?method=updateOrDeleteBig&thisPage=1&search=${notices.search}">首页</a>】
	    		【<a href="${pageContext.request.contextPath}/workerCustomerServlet?method=updateOrDeleteBig&thisPage=${notices.pageNum-1}&search=${notices.search}">上一页</a>】
	    		【<a href="${pageContext.request.contextPath}/workerCustomerServlet?method=updateOrDeleteBig&thisPage=${notices.pageNum+1}&search=${notices.search}">下一页</a>】
	    		【<a href="${pageContext.request.contextPath}/workerCustomerServlet?method=updateOrDeleteBig&thisPage=${notices.totalPage}&search=${notices.search}">尾页</a>】
	    	</c:if>
