...
<head>
	...
	var get = function(){
// 			alert($("#get").val())
			window.location = "${pageContext.request.contextPath}/workerAccountServlet?method=updateOrDelete&search="+$("#get").val()+"&kind=updateOrDelete";
	};
	
	/** 删除信息 */
	var deleteFn = function(id){
		if (confirm("您确定要删除吗？")){
			window.location = "${pageContext.request.contextPath}/workerAccountServlet?method=delete&id=" + id;
		
	};
	}
	/** 修改信息 */
	var updateFn = function(id){
			window.location = "${pageContext.request.contextPath}/workerAccountServlet?method=update&id=" + id;	
	};
		...
		<div id="page-wrapper" class="gray-bg" >
			...
			<div class="row clearfix">
				<div class="col-md-12 column">
                    ...
                    <div class="col-sm-12">
                        <h4 class="example-title">客户资料管理</h4>
                        
                        <div class="toolbar-menu">
                        	<div class="btn-group hidden-xs" id="toolbar" role="group">
            	        	<input id="get" style="margin-right:10px"><button onclick="get()">查询</button> <span style="color:red">${msg}</span>
                        ...
                        <div class="example">
                            <table id="bill-table" data-mobile-responsive="true">
                                    ...
                                    <tr>
                                    	...
                                    	<th data-field="check_id">账户id</th>
                                    	<th data-field="check_id">用户手机</th>
                                        <th data-field="check_id">用户姓名</th>
                                        <th data-field="phone_id">银行账号</th>
                                        <th data-field="account_id">开户银行</th>
                                        <th data-field="check_balance">开户支行</th>
                                        <th data-field="time">账户余额</th>
                                        <th data-field="time">总消费额</th>
                                        <th data-field="time">本月消费额</th>
                                        <th data-field="time">操作</th>
                                        <th data-field="time">操作</th>
                                    </tr>
                                    
                                    <!--                                         迭代数据 -->
											<c:forEach items="${account}" var="r">
                                        <tr class="field" style="border-bottom:#f3d5d6 1px dashed;">
							                ...
							                <td>${r.open_bank}</td>
							                <td>${r.sub_bank}</td>
							                <td>${r.remainder_money}</td>
							                <td>${r.cost_money}</td>
							                <td>${r.month_money}</td>
 											<td><a href="javascript:void(0);" onclick="deleteFn(${r.id});" style="color:blue;">删除</a></td>
	                						<td><a href="javascript:void(0);" onclick="updateFn(${r.id});" style="color:blue;">修改</a></td>
							             </tr>   
							                </c:forEach>
...
<!-- 	    	在第一页，且不止一页，首页，上一页不可点 -->
	    		【首页】
	    		【上一页】
	    		【<a href="${pageContext.request.contextPath}/workerAccountServlet?method=updateOrDelete&thisPage=${notices.pageNum+1}&search=${notices.search}">下一页</a>】
	    		【<a href="${pageContext.request.contextPath}/workerAccountServlet?method=updateOrDelete&thisPage=${notices.totalPage}&search=${notices.search}">尾页</a>】
	    	</c:if>
	    	    		
	    	<c:if test="${notices.pageNum>1 && notices.pageNum<notices.totalPage}">
<!-- 	    	全都可点 -->
	    		【<a href="${pageContext.request.contextPath}/workerAccountServlet?method=updateOrDelete&thisPage=1&search=${notices.search}">首页</a>】
	    		【<a href="${pageContext.request.contextPath}/workerAccountServlet?method=updateOrDelete&thisPage=${notices.pageNum-1}&search=${notices.search}">上一页</a>】
	    		【<a href="${pageContext.request.contextPath}/workerAccountServlet?method=updateOrDelete&thisPage=${notices.pageNum+1}&search=${notices.search}">下一页</a>】
	    		【<a href="${pageContext.request.contextPath}/workerAccountServlet?method=updateOrDelete&thisPage=${notices.totalPage}&search=${notices.search}">尾页</a>】
	    	</c:if>
