...
// 	 		alert($("#address").val());
// 	 		alert($("#manager").val());
			if($("#phone").val() == null || "" == $("#phone").val() || $("#name").val() == null || "" == $("#name").val() 
					|| $("#customerType").val() == null || "" == $("#customerType").val()
					|| $("#address").val() == null || "" == $("#address").val()
					|| $("#manager").val() == null || "" == $("#manager").val()
					|| v == null || "" == v
					|| $("#tc").val() == null || "" == $("#tc").val()){
				alert("输入内容不能为空")
			}else{
		
				window.location = "${pageContext.request.contextPath}/workerCustomerServlet?method=addBigToReserve&phone="
					+$("#phone").val()+"&name="+$("#name").val()+"&customerType="+$("#customerType").val()
					+"&address="+$("#address").val()+"&manager="+$("#manager").val()+"&industry="+v+"&tc="+$("#tc").val();
			}
		...
		<div id="page-wrapper" class="gray-bg" >
			...
			<div class="row clearfix">
				<div class="col-md-12 column">
                    ...
                    <div>
                        <h4 class="example-title">客户资料添加</h4>
                    </div>    
                    	
                				<div style="text-align:center; margin-top:200px; margin-bottom:200px">
                					<div >
                					<span style="font-size:17px; "></span>客户手机：<input id="phone" value="${bigCustomer.phone}">
                					</div>
                					<div style="marigin-top:20px">
                						<span style="font-size:17px"></span>客户姓名：<input id="name" value="${bigCustomer.name}">
                					</div>
                					<div>
                						<span style="font-size:17px"></span>客户类型：<input id="customerType" value="大客">
                					</div>
                					<div>
                					<span style="font-size:17px"></span>客户地址：<input id="address" value="${bigCustomer.address}">
                					</div>
                					<div>
                						<span style="font-size:17px"></span>客户经理：<input id="manager" value="${bigCustomer.manager}">
                					</div>
                					<div>
                						<span style="font-size:17px"></span>客户行业：
                						<select style="width:165px;height:27px" >
	                    					<c:forEach items="${industryType}"  var="i">
		                    					<option id="type" name="type">${i.industry}</option>                    				
		                    				</c:forEach>
  								  		</select>		
  								  		
                					</div>
                						<div>
                						<span style="font-size:17px"></span>电信专员：<input id="tc" value="${bigCustomer.tc}">
                						<br/><button onclick="submit()" style="margin-top:20px">提交</button>
                					</div>
