...
<head>
	...
	var get = function(){
// 			alert($("#get").val())
			window.location = "${pageContext.request.contextPath}/workerCustomerServlet?method=queryBusiness&search="+$("#get").val()+"&kind=query";
	};
</script>
</head>
                            ...
                            <table id="bill-table" data-mobile-responsive="true">
                                    ...
                                    <!--                                         迭代数据 -->
                                        ...
                                        <tr class="field" style="border-bottom:#f3d5d6 1px dashed;">
                                        	<td><input id="checkbox" class="checkbox" value="${r.phone}" id="box_${r.phone}" type="checkbox"/> </td>                                 
							            	<td>${r.id}</td>
							            	<td>${r.phone}</td>
							            	<td>${r.name}</td>
							                <td>${r.type}</td>
...
<!-- 	    	在第一页，且不止一页，首页，上一页不可点 -->
	    		【首页】
	    		【上一页】
	    		【<a href="${pageContext.request.contextPath}/workerCustomerServlet?method=queryBusiness&thisPage=${notices.pageNum+1}&search=${notices.search}">下一页</a>】
	    		【<a href="${pageContext.request.contextPath}/workerCustomerServlet?method=queryBusiness&thisPage=${notices.totalPage}&search=${notices.search}">尾页</a>】
	    	</c:if>
	    	    		
	    	<c:if test="${notices.pageNum>1 && notices.pageNum<notices.totalPage}">
<!-- 	    	全都可点 -->
	    		【<a href="${pageContext.request.contextPath}/workerCustomerServlet?method=queryBusiness&thisPage=1&search=${notices.search}">首页</a>】
	    		【<a href="${pageContext.request.contextPath}/workerCustomerServlet?method=queryBusiness&thisPage=${notices.pageNum-1}&search=${notices.search}">上一页</a>】
	    		【<a href="${pageContext.request.contextPath}/workerCustomerServlet?method=queryBusiness&thisPage=${notices.pageNum+1}&search=${notices.search}">下一页</a>】
	    		【<a href="${pageContext.request.contextPath}/workerCustomerServlet?method=queryBusiness&thisPage=${notices.totalPage}&search=${notices.search}">尾页</a>】
	    	</c:if>
