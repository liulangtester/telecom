...
<head>
	...
	var get = function(){
// 			alert($("#get").val())
			window.location = "${pageContext.request.contextPath}/workerCustomerServlet?method=updateOrDeleteBusiness&search="+$("#get").val()+"&kind=updateOrDelete";
	};
	
	/** 删除信息 */
	var deleteFn = function(id){
		if (confirm("您确定要删除吗？")){
			window.location = "${pageContext.request.contextPath}/workerCustomerServlet?method=deleteBusiness&id=" + id;
		
	};
	}
	/** 修改信息 */
	var updateFn = function(id){
			window.location = "${pageContext.request.contextPath}/workerCustomerServlet?method=updateBusiness&id=" + id;	
	};
                        ...
                        <div class="example">
                            <table id="bill-table" data-mobile-responsive="true">
                                    ...
                                    <tr>
                                    	...
                                    	<th data-field="check_id">客户id</th>
                                        <th data-field="check_id">客户手机号码</th>
                                        <th data-field="phone_id">客户姓名</th>
                                        <th data-field="account_id">客户类型</th>
                                        <th data-field="check_balance">客户地址</th>
                                        <th data-field="time">客户经理</th>
                                        <th data-field="time">客户行业</th>
                                        <th data-field="time">操作</th>
                                        <th data-field="time">操作</th>
                                    </tr>
                                    
                                    <!--                                         迭代数据 -->
											<c:forEach items="${businessCustomer}" var="r">
...
<!-- 	    	在第一页，且不止一页，首页，上一页不可点 -->
	    		【首页】
	    		【上一页】
	    		【<a href="${pageContext.request.contextPath}/workerCustomerServlet?method=updateOrDeleteBusiness&thisPage=${notices.pageNum+1}&search=${notices.search}">下一页</a>】
	    		【<a href="${pageContext.request.contextPath}/workerCustomerServlet?method=updateOrDeleteBusiness&thisPage=${notices.totalPage}&search=${notices.search}">尾页</a>】
	    	</c:if>
	    	    		
	    	<c:if test="${notices.pageNum>1 && notices.pageNum<notices.totalPage}">
<!-- 	    	全都可点 -->
	    		【<a href="${pageContext.request.contextPath}/workerCustomerServlet?method=updateOrDeleteBusiness&thisPage=1&search=${notices.search}">首页</a>】
	    		【<a href="${pageContext.request.contextPath}/workerCustomerServlet?method=updateOrDeleteBusiness&thisPage=${notices.pageNum-1}&search=${notices.search}">上一页</a>】
	    		【<a href="${pageContext.request.contextPath}/workerCustomerServlet?method=updateOrDeleteBusiness&thisPage=${notices.pageNum+1}&search=${notices.search}">下一页</a>】
	    		【<a href="${pageContext.request.contextPath}/workerCustomerServlet?method=updateOrDeleteBusiness&thisPage=${notices.totalPage}&search=${notices.search}">尾页</a>】
	    	</c:if>
